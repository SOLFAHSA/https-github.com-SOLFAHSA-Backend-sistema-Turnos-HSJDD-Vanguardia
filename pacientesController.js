...
/**
 * Obtener un paciente por número de identidad
 */
exports.getPacienteByIdentidad = async (req, res) => {
    console.log("🟢 getPacienteByIdentidad ejecutado.");
    const { identidad } = req.params;

    try {
        const result = await pool.query('SELECT * FROM pacientes WHERE identidad = $1', [identidad]);
        if (result.rows.length === 0) {
            return res.status(404).json(null); // paciente no encontrado
        }
        res.json(result.rows[0]);
    } catch (error) {
        handleError(res, error, "Error al obtener paciente por identidad");
    }
};
